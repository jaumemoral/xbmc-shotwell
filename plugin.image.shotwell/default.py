        ...
        for e in events:
            ...
            xbmcplugin.addDirectoryItem(
                ...
                listitem=xbmcgui.ListItem(
                    name, iconImage=e['icon'], thumbnailImage=e['thumbnail'])
            )
        xbmcplugin.endOfDirectory(self.handle, cacheToDisc=False)
    ...
    def all_tags(self):
        ...
        for t in tags:
            xbmcplugin.addDirectoryItem(
                ...
                totalItems=len(tags),
                listitem=xbmcgui.ListItem(
                    t['name'], iconImage="", thumbnailImage="")
            )
        xbmcplugin.endOfDirectory(self.handle, cacheToDisc=False)

    def tag_pictures(self, tag_id, flagged):
        self.fill_picture_list(
            self.shotwell.picture_list_tag(tag_id, flagged), flagged)

    def last_pictures(self, flagged=False):
        self.fill_picture_list(
            self.shotwell.picture_list_last(flagged), flagged)

    def event_pictures(self, event_id, flagged):
        self.fill_picture_list(
            self.shotwell.picture_list_event(event_id, flagged), flagged)
    ...
    def fill_picture_list(self, l, flagged=True):
        ...
        if not flagged:
            xbmcplugin.addDirectoryItem(
                ...
                totalItems=len(l) + 1,
                listitem=xbmcgui.ListItem(
                    __language__(30003), iconImage="", thumbnailImage="")
            )
        for f in l:
            xbmcplugin.addDirectoryItem(
                self.handle,
                ...
                totalItems=len(l) + 1,
                listitem=xbmcgui.ListItem(
                    f['name'], iconImage=f['icon'], thumbnailImage=f['thumbnail'])
            )
        xbmcplugin.endOfDirectory(self.handle, cacheToDisc=False)
